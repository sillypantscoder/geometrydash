(() => {
	...
	x.addEventListener("loadend", () => {
		...
		for (var i = 0; i < levels.length; i++) {
			...
			e.innerHTML = `<div class="level-title"></div><div>Verified: ${v==100 ? "Yes" : (v==0 ? "No" : `No (${v}%)`)}</div><div>Coins: ${cts.completion.coins.length > 0 ? cts.completion.coins.map((v) => ["[Unverified]", "[Verified]"][v * 1]).join(" ") : "None"}</div><div></div><div><a><img src="../assets/ui/LevelStart.svg"></a></div>`
...
(() => {
	...
	x.addEventListener("loadend", () => {
		...
		for (var i = 0; i < levels.length; i++) {
			...
			e.classList.add("area")
			var v = cts.completion.percentage
			e.innerHTML = `<div class="level-title"></div><div>Verified: ${v==100 ? "Yes" : (v==0 ? "No" : `No (${v}%)`)}</div><div>Coins: ${cts.completion.coins.length > 0 ? cts.completion.coins.map((v) => ["[Unverified]", "[Verified]"][v * 1]).join(" ") : "None"}</div><div></div><div><a><img src="../assets/ui/LevelStart.svg"></a><a><img src="../assets/ui/LevelEdit.svg"></a></div>`
			e.children[0].innerText = cts.name
			e.children[3].innerText = cts.description
